% =========================================================================
% NPL finite-domain quantified paper — draft v0.1 (2026-07-09).
...
Draft v0.1 (July 2026). Companion to \emph{Nullivance Propositional Logic:
...
Appendix~\ref{app:lean}. The related-work section is a stub pending the
project's literature pass; novelty wording is correspondingly guarded.}}
...
lifts to the quantified language over finite domains; (ii)~a natural signed
tableau calculus is \emph{refuted} as complete --- crisp equality
$x{=}x$ is valid but underivable --- and two further tempting proof routes
are refuted by explicit countermodels, all machine-checked; (iii)~the
...
bit of the value, exactly as in the propositional case. A \emph{signed
branch member} is a triple (sign, assignment, formula); a model satisfies
a branch iff it satisfies every member.
[L:\,\lean{FiniteFO.qeval}, \lean{forallV4}, \lean{existsV4},
\lean{qsatSigned}, \lean{qsatBranch}]
\end{definition}
...
\section{Grounding}\label{sec:groundingdef}
...
The propositional tableau rules lift to signed quantified branch members,
with four finite quantifier rules per quantifier: the \emph{block} rules
add all $n{+}1$ instances to the branch at once (non-branching: $\Tp\forall$,
$\Fm\forall$, $\Tm\exists$, $\Fp\exists$), and the \emph{per-element}
rules require one closed child per domain element (the remaining four
signs). Closure, in the naive calculus, is an identical opposite-signed
pair. [L:\,\lean{FiniteFO.QCloses}]
...
strictly smaller weight, and monotonicity of closure restores the full
...
\emph{This section is a stub pending the project's systematic literature
pass; wording is deliberately guarded.} Signed tableaux with
sets-of-values signs for finitely-valued logics, including quantified
ones, are a standard paradigm \cite{haehnle1994automated}; quantified
Belnap--Dunn logics and their semantics are classical territory
\cite{belnap1977useful,dunn1976intuitive,priest2008introduction}. The
NPL-specific ingredients here are the threshold-signed presentation with
the harmonization connective in the object language, the crisp-equality
and ground-closure clause set of the core calculus, the domain-weighted
completeness engine, and the machine-checked status of the entire
development including its refutations. Which of these ingredients appear
elsewhere — in particular in the literature on tableaux for
finitely-valued first-order logics and on formalized completeness
theorems \cite{villadsen2017formalizing} — is exactly what the pending
literature pass must establish before any novelty claim is made.
...
part of the same artifact as the companion paper. \textbf{Toolchain.}
