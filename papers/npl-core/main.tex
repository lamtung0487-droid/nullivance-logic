% =========================================================================
% NPL core paper — release v0.7.1 (2026-08-16).
% v0.7.1: citation corrections; public repository and archival DOI synchronization.
% v0.7.0: stable Lean/mathlib release pair; reproducibility gates; artifact and
% related-work refresh; finite-FO companion status synchronized.
...
\hypersetup{
  pdftitle={Nullivance Propositional Logic: Threshold-Signed Consequence on the Unit Square with a Machine-Checked Completeness Theorem},
  pdfauthor={Trinh Tung Lam},
  pdfsubject={A formally verified propositional logic with threshold-signed consequence},
  pdfkeywords={nullivance logic, paraconsistent logic, many-valued logic, completeness, Lean 4}
}
...
Release v0.7.1 (August 2026). Results marked [L] are verified in the Lean~4 artifact
described in Appendix~\ref{app:lean}; results marked [paper] carry full proofs
here and are not yet formalized. Complete verification and citation metadata are
...
(v)~an optional generative tier, logically independent of the core, derives truth-objects from
...
cutworthiness of $\alpha$-cuts \cite[\S\S~1.3--1.4]{klir1995fuzzy}; sets-as-signs
signed tableaux for finitely-valued logics are standard
\cite{haehnle1994automated}, as are signed tableaux for FDE
\cite[Ch.~8]{priest2008introduction}. Moreover --- a comparison we performed rather than
...
\item \textbf{A calibrated failure map} (\cref{sec:profile}): explosion,
  ...
  (\cref{prop:primitive}).
\item \textbf{An optional verified generative tier} (\cref{sec:generative}):
...
Results carry [L:\,\texttt{declaration}] markers naming the verifying Lean
...
\begin{leanrefs}
[L:\,\lean{eval\_mem}, \lean{latent\_collapse},
\lean{oplus2\_comm}, \lean{oplus2\_assoc}, \lean{oplus2\_idem},
\lean{neg2\_oplus2}, \lean{B2\_oplus}, \lean{oplus2\_B2},\newline
\lean{conj2\_le\_t\_left}, \lean{conj2\_le\_t\_right},
\lean{le\_t\_conj2}, \lean{disj2\_le\_t\_left},
\lean{disj2\_le\_t\_right}, \lean{disj2\_le\_t},\newline
\lean{oplus2\_le\_k\_left}, \lean{oplus2\_le\_k\_right},
\lean{le\_k\_oplus2}, \lean{N2\_le\_k}, \lean{le\_k\_B2}]
\end{leanrefs}
\end{lemma}
...
\cite[\S\S~1.3--1.4]{klir1995fuzzy}; negation commutes with the two cuts because it swaps
...
finite computation on the tables.
\end{proof}

\begin{definition}[Reference search and progressing forest transition]
\label{def:reference-search}
For a compound signed formula $c$, let
$\mathrm{child}(c)=[\Delta_1,\ldots,\Delta_k]$ be the ordered list of
one or two child blocks printed for $c$ in \cref{def:calculus}. Define the
terminal-leaf function on a worklist $W$ and atomic accumulator $L$ by
\[
\begin{aligned}
\operatorname{run}([],L)&=[L],\\
\operatorname{run}(c::W,L)&=\operatorname{run}(W,c::L)
 &&\text{if $c$ is atomic},\\
\operatorname{run}(c::W,L)&=
 \mathop{+\!+}_{i=1}^{k}\operatorname{run}(\Delta_i+\!+W,L)
 &&\text{if $c$ is compound}.
\end{aligned}
\]
Here $+\!+$ is list concatenation and the outer concatenation preserves the
rule-table order. The Boolean reference procedure returns true iff every
$L\in\operatorname{run}(B,[])$ contains an opposite-sign atomic pair.

A forest state is a finite list of pairs $(W,L)$. One \emph{progressing
forest step} selects one pair with $W\ne[]$ and replaces it by $(W',c::L)$
in the atomic case, or by all $(\Delta_i+\!+W',L)$ in the compound case,
where $W=c::W'$. It never permits an idle step. A forest is terminal iff
all its worklists are empty, and closed iff every terminal accumulator
contains an opposite-sign pair.
[L:\,\lean{Operational.run}, \lean{referenceCloses},
\lean{ForestStep}, \lean{ForestTerminal}, \lean{ForestClosed}]
\end{definition}

\begin{theorem}[Verified reference and schedule-independent proof search]\label{thm:reference-search}
For every finite signed branch $B$, the deterministic head-worklist
procedure terminates and returns true exactly when $B$ is unsatisfiable,
equivalently exactly when \lean{Closes} $B$. If it returns false, one of
its terminal atomic leaves determines a canonical $\FOUR$-valuation
satisfying both that leaf and $B$. More generally, place the active branch
states in a finite forest and repeatedly select any nonterminal state,
replacing it by all of its children. Every such progressing selection
order terminates; all terminal outputs agree on closure and are closed
exactly when $B$ is unsatisfiable.
[L:\,\lean{refStep\_wellFounded},
\lean{satBranch\_constraints\_iff\_run},
\lean{referenceCloses\_iff\_closes},
\lean{referenceCloses\_false\_countermodel},
\lean{forestStep\_wellFounded},
\lean{terminal\_reachable\_closed\_iff\_closes},
\lean{terminal\_outputs\_agree}]
\end{theorem}
\begin{proof}
A state consists of a list of formulas still to process and an atomic
accumulator. The scheduler removes the head occurrence, moves an atom to
the accumulator, and replaces a compound by the products of its signed
tableau rule; both children of a branching rule are explored. The sum of
the sizes of the unprocessed formula occurrences decreases strictly in
every child, proving termination.

For arbitrary active-branch selection, define the cost of a branch state
as one plus the sum of the costs of all its immediate children, and sum
these costs over the current forest. Replacing one selected state by all
of its children decreases this global cost by exactly one. Hence the
forest transition is well founded independently of the selection order;
no fairness premise is needed because an allowed transition must make
progress rather than stutter.

Induction over this well-founded recursion shows that the current
constraints are satisfiable iff at least one returned terminal leaf is
satisfiable. Every returned leaf is atomic. An atomic leaf is
unsatisfiable iff it contains an opposite-sign pair: in the open case, the
valuation whose truth and falsity bits record the positive atomic signs
satisfies every member. Hence all returned leaves close iff the input is
unsatisfiable; tableau completeness identifies this condition with
\lean{Closes}. On failure, the same canonical valuation of an open leaf,
together with the recursion invariant, is the asserted countermodel.
The corresponding one-step forest invariant is preserved along every
finite progress trace. Thus every reachable terminal forest has the same
closure verdict, and an open terminal member transports its canonical
valuation back to a countermodel of the input.
\end{proof}
...
coNP-complete because Boolean satisfiability is NP-complete and tautology is
its complement under negation; see
\cite{cook1971complexity,karp1972reducibility}.
\end{proof}

\begin{theorem}[Compactness; strong completeness]\label{thm:compactness}
\mbox{}\par\noindent
\textnormal{\small [L:\,\lean{compactness\_satisfiable4\_set},
\lean{compactness\_consequence4\_set},}\par\noindent
\textnormal{\small
\lean{compactness\_consequenceC\_set},
\lean{derivesSet\_iff\_consequenceCSet}]}\par\noindent
...
\section{An optional generative tier}\label{sec:generative}

This section is a mathematical interface, not an empirical claim about how
support is generated in applications. Nothing in
\cref{sec:syntax,sec:semantics,sec:projection,sec:calculus,%
...
system (the structure coordinate cannot be recovered from a measure, such as
Shannon entropy, computed only from the effective scalar probabilities
...
bifilter of $[0,1]\odot[0,1]$. Indeed $D_\tau$ is nonempty and proper;
writing $a_t$ for the truth coordinate,
\[
 a\wedge b\in D_\tau\ \Longleftrightarrow\
 a\otimes b\in D_\tau\ \Longleftrightarrow\
 \min(a_t,b_t)\ge\tau\ \Longleftrightarrow\
 a,b\in D_\tau,
\]
which is the bifilter condition, while
$a\vee b\in D_\tau\Longleftrightarrow
\max(a_t,b_t)\ge\tau\Longleftrightarrow a\in D_\tau$ or
$b\in D_\tau$, which is primeness. Thus
...
$\alpha$-cut cutworthiness \cite[\S\S~1.3--1.4]{klir1995fuzzy}. \textbf{Tableaux.}
Sets-as-signs tableaux are standard \cite{haehnle1994automated}; FDE has
textbook signed tableaux \cite[Ch.~8]{priest2008introduction}. The closest tableau
...
audit; last sweep August 2026, covering post-2022 twist-product,
...
completeness theorem for a macro-free extensional tableau calculus, documented
in the companion manuscript included in this release. Open:
...
\textbf{Availability.} The complete source artifact is supplied with this
release and contains \texttt{CITATION.cff}, \texttt{LICENSE},
\texttt{ARTIFACT.md}, the verification scripts, and both manuscripts. The
tagged source archive is available from
...
\textbf{Toolchain.} Lean 4
(\texttt{leanprover/lean4:v4.32.1}) and mathlib \texttt{v4.32.1} at commit
\texttt{520045ab14e26149ee970e2e617ca04b09bde5d6}; build with
\texttt{lake exe cache get \&\& lake build}, or run the repository-level
\texttt{scripts/Verify-Release.ps1} gate.
...
\cref{prop:bifilter} ---
carry full proofs in this paper and are not formalized. The verified
operational claim covers the deterministic reference procedure and every
progressing active-branch selection order of \cref{thm:reference-search}.
It does not assert weak fairness for a different transition system that
permits idle steps. \textbf{Modules.} \texttt{Syntax}, \texttt{Semantics}
...
\texttt{Operational} (terminating reference search and countermodels),
