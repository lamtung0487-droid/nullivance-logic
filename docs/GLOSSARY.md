...
| Atomic branch | — | Def 3.79 | `Operational.AtomicBranch` | Branch every formula occurrence of which is an atom |
...
| Admissible core replay closure | — | Def 3.44; Prop 3.45; Conj 3.50 | `FiniteFO.ReplayClosesCore`, `FiniteFO.ReplayClosesCore.toCore` | Sound head-sensitive trace-level certificate; Conj 3.50 refutes completeness for all admissible ground-closed traces |
| Admissible ground-to-replay certificate bridge | — | Conj 3.50 | `FiniteFO.admissible_ground_replay_bridge_refuted` | Refuted claim that every admissible propositionally closed trace has a `ReplayClosesCore` certificate; the nonempty cascade witness isolates head-sensitive residual-fold blocking |
...
| Admissible replay semantic bridge | — | Thm 3.77 | `FiniteFO.ReplayTrace.admissible_ground_closes_to_core` | Ground closure of an admissible trace implies core closure of its quantified projection, without constructing a replay certificate |
...
| Consequence (hệ quả) | ⊨ | Def 2.6 | arbitrary sets: `Metatheory.Consequence4Set`, `ConsequenceCSetModel`; finite lists: `Consequence4`, `ConsequenceCModel`; fixed τ: `ConsequenceCAt` | Preservation of signed satisfaction over every bundled model; equivalent unbundled APIs are certified by Thm 2.29; τ-invariant (Prop 4.27) |
...
| Idle-step weak fairness (future) | — | Thm 4.33 scope note; DR-0018 | — | Optional liveness condition needed only for a transition system that permits stuttering; not asserted by the progress-scheduler theorem |
...
| Flat replay compiler | — | Prop 3.88–3.90 | `FiniteFO.ReplayTrace.flat_qLits_closes_to_replay`, `FiniteFO.ReplayTrace.membership_bridge_of_flat_compiler`, `FiniteFO.ReplayTrace.flat_closes_to_replay` | Reifies a closed flat normal form as an old replay certificate; the literal source-inversion base and the full domain-weighted compiler are verified |
| Flat replay normal form | `flatFor(T,B)`, `flatBranch(T)` | Def 3.85 | `FiniteFO.ReplayTrace.flatFor`, `FiniteFO.ReplayTrace.flatBranch` | Four fold identities plus a grounded quantified worklist plus exactly the rigid items retained by the trace |
| Fixed signature | Σ | Def 2.25 | `FiniteFO.QSignature` | Assigns one natural-number arity to every predicate symbol |
| Fold-first membership saturation | — | Prop 3.86 | `FiniteFO.ReplayTrace.membership_saturation_elim` | Finite normalization that materializes every quantified-projection member as a direct `q` item before embedding the old replay certificate |
| Signature-indexed finite FOUR model | S | Def 2.27 | `FiniteFO.QSigModel` | Interprets each predicate only on tuples of its signature-declared arity |
| Signature-indexed finite consequence | `QConsequence4Sig_Σ` | Def 2.27 | `FiniteFO.QConsequence4Sig` | Signed consequence over genuine signature-indexed finite FOUR models, restricted to well-formed inputs |
...
| Forest closed | `ForestClosed(F)` | Def 3.80 | `Operational.ForestClosed` | Every terminal atomic accumulator in the search forest contains an opposite-sign pair |
| Forest cost | `C(F)` | Def 3.80 | `Operational.forestCost` | Sum of remaining full expansion-tree sizes; decreases on every progressing forest step |
| Forest satisfaction | `ForestSat(v,F)` | Def 3.80 | `Operational.ForestSat` | Some branch state in the forest has represented constraints satisfied by `v` |
| Forest terminal | `Terminal(F)` | Def 3.80 | `Operational.ForestTerminal` | Every branch state in the forest has an empty worklist |
...
| Membership-selecting core replay certificate | — | Def 3.81; Prop 3.82–3.83 | `FiniteFO.ReplayClosesCoreMem`, `FiniteFO.ReplayClosesCoreMem.toCore`, `FiniteFO.ReplayClosesCore.toMem` | Conservative extension of the head-sensitive replay certificate that selects structured fold occurrences and branching alternatives by list membership |
| Meta-sign | T⁺,T⁻,F⁺,F⁻ | Def 2.4 | `Sign`, `V4.sat`; `Continuous.SatC` | Threshold predicate on a single channel of a formula's value |
| Model (mô hình) | M = (v, τ) | Def 2.2 | `Continuous.Model`; conversions `Model.ofSquareValuation`, `Model.squareValuation` | Atom valuation into `[0,1]²` plus a threshold in `(0,1]`; bundled/unbundled equivalence is Thm 2.29 |
| Model agreement on a signature | M ≡_Σ N | Lem 2.26 | `FiniteFO.QModel.AgreeOn` | Models have equal predicate values on every tuple whose length is declared by Σ |
...
| Progressing forest step | `F →_F G` | Def 3.80 | `Operational.ForestStep` | Selects any nonterminal forest occurrence and replaces it by all of its reference children |
| Progressing scheduler | σ | Def 3.80 | `Operational.ProgressScheduler` | Chooses a legal progressing successor whenever the current forest is nonterminal; cannot stutter |
| Progress trace | `F →_F* G` | Def 3.80 | `Operational.ForestReach` | Finite reflexive-transitive sequence of progressing forest steps |
...
| Reference closure test | `referenceCloses(B)` | Def 3.79 | `Operational.referenceCloses` | Boolean test true exactly when every terminal branch returned by reference execution contains an opposite-sign pair |
| Reference execution | `run(todo,lits)` | Def 3.79 | `Operational.run` | Exhaustive left-first depth-first recursion returning every terminal atomic branch |
| Reference search branch | `W=(todo,lits)` | Def 3.78 | `Operational.SearchBranch` | Operational state separating unprocessed signed occurrences from accumulated atomic constraints |
| Reference transition | `W →_ref W'` | Def 3.78 | `Operational.RefStep`, `Operational.children` | One deterministic-head decomposition step; branching rules expose both child states |
...
| Search forest | F | Def 3.80 | `Operational.SearchForest` | Finite list of operational branch occurrences among which a progressing scheduler may choose |
...
| Raw truth-object carrier | ℝ×ℝ | Thm 2.29 | `Continuous.TruthObj`, predicate `InSquare` | Ambient implementation carrier; includes out-of-square pairs and is not itself the semantic domain |
| Truth-object (vật chân lý) | (t, f) | Def 2.1 | `Continuous.SquareTruthObj`; raw representation `TruthObj` + `InSquare`; FOUR fragment `V4` | Pair of independent channel values in `[0,1]²` |
| Universal membership replay bridge | — | Conj 3.84 | `FiniteFO.admissible_ground_replay_bridge_mem_verified` | Verified theorem that every admissible ground-closed replay trace has a membership-selecting core replay certificate; it combines fold saturation, flat semantic reduction, and the full flat compiler |
...
| Well-formed quantified formula | WF_Σ(φ) | Def 2.25 | `FiniteFO.QFormula.WellFormed` | Every predicate occurrence has the arity assigned by the fixed signature Σ |
| Worklist weight | `μ(W)` | Def 3.78 | `Operational.SearchBranch.weight`, `Metatheory.weight` | Sum of formula sizes of all unprocessed list occurrences; strictly decreases on every reference transition |

## Naming conventions

- English terminology is canonical; explanatory Vietnamese equivalents may appear in parentheses.
